    ...
    [void] Seeking() {
        ...
        while ($private:index -lt $this.queue.count -and $private:index -ge 0){
            ...
            if ($private:index -eq $this.queue.count-1) {
                $private:now=(Get-Date)
                Write-Host ("{0:yyyy}-{0:MM}-{0:dd} {0:HH}:{0:mm}:{0:ss}.{0:fff}" -f $private:now)
                Write-Host ("{0:HH}:{0:mm}:{0:ss}.{0:fff} -- {1:HH}:{1:mm}:{1:ss}.{1:fff}" -f $this.starttime, $private:now)
                Write-Host ("<PowerShell>:[{0:hh}:{0:mm}:{0:ss}.{0:fff}]" -f (New-TimeSpan -Start $this.starttime -End $private:now))
...
Write-Host
$private:now=(Get-Date)
Write-Host ("{0:yyyy}-{0:MM}-{0:dd} {0:HH}:{0:mm}:{0:ss}.{0:fff} -- {1:yyyy}-{1:MM}-{1:dd} {1:HH}:{1:mm}:{1:ss}.{1:fff}" -f $starttime, $private:now)
Write-Host ("<PowerShell>:[{0:hh}:{0:mm}:{0:ss}.{0:fff}]" -f (New-TimeSpan -Start $starttime -End $private:now))
